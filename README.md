...
<p align="center">
  Convert images into terminal-native block art, font-matched glyph art, or fuse-bead patterns.<br>
...
The primary showcase uses [`docs/assets/shinku02.jpg`](docs/assets/shinku02.jpg) as one shared source, so differences
between renderers come from their representation strategy rather than different input artwork. A second
wide-format source demonstrates higher-resolution bead grids below.

## Why EdgeGlyph

Most image-to-ASCII tools map brightness to a character ramp. EdgeGlyph separates three different visual
problems instead of forcing them through one heuristic:
...
- **Bead mode is cell-first.** It assigns one physical color to each square-grid position, removes empty
  background cells, and renders a high-resolution pegboard preview with real bead geometry.

All modes return the same result model and support plain text, color PNG, NvDash Lua, debug images, and
...
### Fuse-bead patterns and physical previews

Bead mode converts each grid cell into one round fuse bead, limits the palette in OKLab, reports material
counts, and renders light, dark, or transparent pegboards with glossy or matte finishes.

<p align="center"><img src="docs/example-bead-render.png" width="72%" alt="EdgeGlyph physical fuse-bead preview generated from shinku02.jpg"></p>

For detailed patterns, the grid can be increased to `2048 × 2048` with as many as `128` colors. The source sampling and PNG bead size
adapt automatically at large dimensions, while every requested logical grid cell remains available.

<p align="center">
  <img src="docs/assets/atri1.png" width="47%" alt="Wide-format source artwork for the EdgeGlyph bead example">
  <img src="docs/atri1-bead-render.png" width="47%" alt="96 by 56 dark pegboard preview generated by EdgeGlyph">
</p>
<p align="center"><sub>A wide-format 96 × 56 pattern rendered with 16 colors on a dark matte board.</sub></p>
...
| Argument | Purpose | Default |
| --- | --- | ---: |
| `--cols`, `--rows` | Logical bead grid dimensions, up to `2048 × 2048` | `48 × 48` |
| `--colors` | Maximum number of bead colors, up to `128` | `12` |
| `--subject-threshold` | Cell coverage required to place a bead | `0.20` |
| `--oversample` | Source samples along each bead-cell axis | `6` |
| `--fit` | `cover` the frame or `contain` the complete source | `cover` |
| `--focus-y` | Vertical crop anchor from top to bottom | `0.5` |
| `--zoom` | Subject scale inside the bead frame | `1.0` |
| `--background` | Automatically remove the background or `keep` it | `auto` |
| `--board-style` | `light`, `dark`, or `transparent` pegboard | `light` |
| `--finish` | `glossy` or `matte` bead surface | `glossy` |
| `--bead-size` | Target pixels per bead; large grids are previewed adaptively | `16` |
...
Run `edgeglyph block --help`, `edgeglyph bead --help`, `edgeglyph glyph --help`, or `edgeglyph schema` for the complete validated
...
`edgeglyph.modes.bead`, `edgeglyph.modes.block`, or `edgeglyph.modes.glyph`.

Bead patterns use the same API shape:
...
Image
  ...
  ├─ bead  → background flood → square-cell sampling → OKLab palette → physical pegboard
...
The test suite covers geometry primitives, block color boundaries, physical bead previews, font-matched rendering, schema
