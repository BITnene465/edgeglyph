...
所有视觉示例均使用 [`docs/shinku02.jpg`](docs/shinku02.jpg) 作为同一源图，因此两种结果之间的差异来自渲染策略，而不是输入素材。

## 为什么选择 EdgeGlyph

大多数图像转 ASCII 工具只按亮度映射字符。EdgeGlyph 将需求拆分为两个不同的视觉问题，避免用同一种启发式算法勉强处理所有图像：

- **色块模式以区域为核心。** 使用空格和 Unicode `▀▄█` 保存实心轮廓与源图颜色，每个终端单元格可以容纳上下两个独立着色的像素。
- **字符模式以结构为核心。** 提取图像边缘，栅格化指定终端字体，匹配局部几何结构，并在完整网格上优化线条连续性。

两种模式使用相同的结果模型，均可输出纯文本、彩色 PNG、NvDash Lua、调试图层和 JSON 指标。所有公开参数来自同一份 schema，CLI 与浏览器控件不会悄然产生差异。
...
运行 `edgeglyph block --help`、`edgeglyph glyph --help` 或 `edgeglyph schema` 可以查看完整且经过验证的接口。0.4 版本之前的 `edgeglyph input.png --style block ...` 写法仍然兼容。
...
`edgeglyph.engine` 中的旧版导入路径仍然可用，但新集成应使用 `edgeglyph.modes.block` 或 `edgeglyph.modes.glyph`。
...
测试涵盖几何基础函数、色块边界、字体匹配渲染、schema 校验、CLI 兼容性与工作台的全部下载格式。修改渲染器默认值或新增公开参数前，请阅读 [CONTRIBUTING.md](CONTRIBUTING.md)。
